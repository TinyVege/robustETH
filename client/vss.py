...
def share(secret: int, n: int, t: int) -> Tuple[List[int], List[PointG1]]:
    ...
    shares = [f(id) for id in range(0, n)]          # id 需要从0开始，一共n个share
...
def recover_point(id_and_point_list: List[Tuple[int, PointG1]]) -> PointG1:
    ...
    for i, sig in id_and_point_list[1:]:
        ...
        result = add(result, t)                 # 加法。普通加法只是聚合签名，而用上lagrange就是门限签名
    return result
...
def encrypt_share(share: int, receiver_id: int, shared_key: PointG1) -> int:            # shared_key = kij
    """ encrypts a share given the provided shared key                                  实际上 H(kij||j) 中的kij = kij[0] == shared_key[0]
    ...
    sx = shared_key[0]
    sx = (sx.to_bytes(32, "big") + receiver_id.to_bytes(32, "big"))
    hx = keccak_256(abi_types = ['bytes'],values = [sx])       # .digest()删掉了
...
def shared_key_proof(sk, other_pk: PointG1) -> Tuple[int, int]:
    ...
    # print(f"G1:{G1}, pk:{pk}, other_pk:{other_pk}, shared_key:{shared_key}, alpha:{sk}")
...
def prove_sk_knowledge(sk: int, pk: PointG1, account: str = None) -> Tuple[int, int]:
    ...
    c = keccak_256(abi_types=types, values=values)
...
def dleq_verify(g1: PointG1, h1: PointG1, g2: PointG1, h2: PointG1, challenge: int, response: int):
    # dleq_verify(g1, g1si = h1, h1 = g2, h1si = h2, c, r))
    temp1 = multiply(g1, response)
    temp2 = multiply(h1,challenge)
    a1 = add(temp1, temp2)

    temp1 = multiply(g2, response)
    temp2 = multiply(h2,challenge)
    a2 = add(temp1, temp2)
    c = keccak_256(  # pylint: disable=E1120
        abi_types=["uint256"] * 12,  # 12,
        values=[
